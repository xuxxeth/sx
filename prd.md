...
- 登录流程需获取后端挑战串并签名，后端校验后下发带过期时间的 token
- token 过期前后端均需校验，过期需重新签名
...
**需求**
- 发帖链下存储，链上记录 CID 与索引
- 图片上传到 IPFS，正文与图片 CID 组合成 JSON 上链（仅 1 个 CID）
- 支持“置顶”
- 支持“仅关注者可见”（链下权限控制）
...
- post_id
- author
- content_cid（指向 JSON：{ content, imageCid? }）
- created_at
- visibility
...
**需求**
- 链下生成与排序，返回互动计数（likeCount/commentCount）
- 支持过滤：关键词、屏蔽用户
